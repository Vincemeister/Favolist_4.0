...
class ProductsController < ApplicationController
  ...
  def show
    ...
    unless @product.viewable_by?(current_user)
      ...
      @suggested_products = Product.joins(:user)
  ...
  def product_params
    params.require(:product).permit(:title, :price, :price_cents, :price_currency, :subscription_type, :review, :description, :url, :list_id, :currency, :logo, :position, photos: [], referral_attributes: [:id, :code, :details])
  end
...
def trigger_list_update
  if @product.list.present?
    ...
    RegenerateBackgroundWorker.perform_async(@product.list.id)
  end
end
